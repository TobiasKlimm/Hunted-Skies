...
void CGame::Init(HWND hwnd, void(*procOS)(HWND hwnd, unsigned int uWndFlags), CSplash* psplash)
{
	...
	// Die ??? und die Geisterinsel
	m_zs.AddPlacement(&m_zpLandscapeHapt);
	m_zpLandscapeHapt.AddGeo(&m_zgLandscapeGhost);

	// LOD der Geisterinsel
	m_zpLandscapeHapt.SetLoD(0.0f, 0.5f);

	m_zgsCollision.Add(&m_zgLandscapeGhost);
	...
	m_zgLandscapeGhost.CreateField(TERRAIN_SIZE, TERRAIN_SIZE, 100, 100, 0.f, 0.f, 10.f, 10.f);
...
void CGame::Tick(float fTime, float fTimeDelta)
{
	...
	unsigned int uNrOfBullets = m_zpsBullets.RingCount();
	CPlacement* pzpLastBullet = m_zpsBullets.RingAskLast();
	if (uNrOfBullets == m_zpsBullets.m_uPlacements)
	{
		for (unsigned int u = 0;
			u < m_zpsBullets.m_uPlacements;
			u++)
		{
			CHVector vPosBullet = m_zpsBullets.m_applacement[u]->GetPos();
			float fHeightTerrain = m_zgTerrain.GetHeight(vPosBullet.x, vPosBullet.z);
			if (vPosBullet.y <= fHeightTerrain)
				m_zpsBullets.m_applacement[u]->SwitchOff();
	...
	{
		for (unsigned int u = m_zpsBullets.m_uRingStart-m_zpsBullets.m_uRingCount;
			u != (m_zpsBullets.m_uRingStart + 1u) % m_zpsBullets.m_uPlacements;
			u = (u + 1) % m_zpsBullets.m_uPlacements)
		{
			CHVector vPosBullet = m_zpsBullets.m_applacement[u]->GetPos();

			LogDebug("u: %u", u);
			LogDebugVector("vPosBullet", vPosBullet);
			float fHeightTerrain = m_zgTerrain.GetHeight(vPosBullet.x, vPosBullet.z);
			LogDebug("fHeight: %f", fHeightTerrain);
			if (vPosBullet.y+0.01f < fHeightTerrain)
			{
				m_zpsBullets.m_applacement[u]->SwitchOff();
				LogDebug("Whaaam!");
			}
